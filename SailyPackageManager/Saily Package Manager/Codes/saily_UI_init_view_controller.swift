...
class saily_UI_init_view_controller: UIViewController, LTMorphingLabelDelegate {
    ...
    override func viewDidLoad() {
        ...
        DispatchQueue.main.async {
            ...
            self.tipLabel.text = GVAR_device_info_identifier_human_readable + " - " + GVAR_device_info_identifier + " - " + GVAR_device_info_current_version
            if (GVAR_device_info_UDID == "") {
                self.tipSubLabel.text = ""
            }else{
                self.tipSubLabel.text = GVAR_device_info_UDID.dropLast(10) + "**********"
            }
